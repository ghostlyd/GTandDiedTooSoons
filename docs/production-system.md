...
6. **Composition layer**: standalone track briefs in `compositions/down-tempo-punk-bluegrass-set.json` plus machine-checkable mutation blueprints in `compositions/composition-mutation-blueprints.json`.
7. **Generated sketch layer**: deterministic Live-import build plans and MIDI sketches in `compositions/generated/`, rendered by `scripts/render_composition_sketches.py`. The build plans mirror the mutation blueprints so each track carries full bluegrass-role jobs, alien-electronic role jobs, punk constraints, source-deck approval state, and Max for Live mutation lanes.
...
- Composition mutation blueprints are metadata-only. They may describe role jobs, section intent, Max for Live lanes, and source-deck review state, but they must not contain raw audio paths, direct source files, private recordings, credentials, or validated psychological-effect claims.
...
- Generated library installation queues and preflight receipts are metadata-only operator handoffs. They may point to official Ableton/Arturia pages and local receipt roots, but they must not perform vendor login, purchase, install, DAW launch, OpenAI API calls, or commit account-bound assets.
